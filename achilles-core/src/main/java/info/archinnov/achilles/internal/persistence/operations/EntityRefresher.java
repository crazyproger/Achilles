...
public class EntityRefresher {
    ...
    public <T> AchillesFuture<T> refresh(T proxy, final EntityOperations context) throws AchillesStaleObjectStateException {
        ...
        final EntityInterceptor<T> interceptor = proxifier.getInterceptor(proxy);
        ...
        Function<T, T> updateInterceptor = new Function<T, T>() {
            @Override
        ...
        return new AchillesFuture<>(Futures.transform(entityFuture, updateInterceptor, context.getExecutorService()));
    }
}
